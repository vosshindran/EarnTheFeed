(function () {
  ...
  function getSettings() {
    ...
    return settings
      ? JSON.parse(settings)
      : { duration: 15, blockedSites: DEFAULT_SITES.map((s) => s.id) };
  }
  ...
  function createTimerUI() {
    const updateTimer = () => {
      ...
      if (remaining <= fiveMinutes) {
        ...
        document.getElementById('etf-time-left').innerText =
  ...
  async function lockPage(isTimeout = false) {
    ...
    const renderSettings = () => {
      ...
      slider.oninput = () =>
        (overlay.querySelector('#dur-val').innerText = slider.value);
        ...
        btn.onclick = async () => {
          ...
          try {
            const res = await fetch(
              `http://127.0.0.1:3000/get-challenge?type=${type}`,
            );
            ...
            codeEl.style.display =
              isWorkout || (!data.code && type !== 'puzzle') ? 'none' : 'block';
            visualizer.style.display = isWorkout ? 'flex' : 'none';
            inputEl.placeholder = isWorkout
              ? "Type 'DONE' when finished..."
              : 'Your answer...';
            if (isWorkout) {
              const isPushup = data.question.toLowerCase().includes('pushup');
              visualizer.innerHTML = characterSVG(
                isPushup ? 'pushup-anim' : 'jack-anim',
                isPushup,
              );
            }
        ...
        try {
          ...
          if (res.ok) {
            ...
            localStorage.setItem(
              'etf-unlock-time',
              Date.now() + (settings.duration || 15) * 60000,
            );
            location.reload();
          }
        } catch (err) {}
    ...
    const muteInterval = setInterval(() => {
      if (!document.getElementById('etf-overlay')) {
